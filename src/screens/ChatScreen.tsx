...
const sendMessageToAI = async (
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        text: "L'IA a été désactivée conformément à votre demande. (Mode local)",
        sender: 'ai',
        type: 'text' as const
      });
    }, 500);
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        reader.onloadend = async () => {
          ...
          // Local mock for Transcribe without AI
          setTimeout(() => {
            setInput(prev => (prev ? prev + ' ' : '') + "(Transcription vocale locale via Web Speech API non implémentée, IA désactivée)");
